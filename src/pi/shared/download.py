...
"""Download utilities for models and datasets."""
...
def _ensure_permissions(path: pathlib.Path) -> None:
    """Since we are sharing cache directory with containerized runtime as well as training script.

    we need to ensure that the cache directory has the correct permissions.
    """
...
def _get_mtime(year: int, month: int, day: int) -> float:
    ...
    date = datetime.datetime(year, month, day, tzinfo=datetime.timezone.utc)  # Python 3.10 compatibility
