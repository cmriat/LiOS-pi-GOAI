...
from pi.models.gemma_.modeling_gemma import GemmaModel
from pi.models.siglip.modeling_siglip import SiglipVisionModel
...
class PaliGemmaCausalLMOutputWithPast(ModelOutput):
    r"""loss (`torch.FloatTensor` of shape `(1,)`, *optional*, returned when `labels` is provided).
...
class PaliGemmaModel(PaliGemmaPreTrainedModel):
    ...
    def __init__(self, config: PaliGemmaConfig):
        ...
        self.vision_tower = SiglipVisionModel._from_config(config.vision_config)
        # self.vision_tower = SiglipVisionModel(config.vision_config)
        self.multi_modal_projector = PaliGemmaMultiModalProjector(config)
        self.vocab_size = config.text_config.vocab_size

        language_model = GemmaModel._from_config(config.text_config)
